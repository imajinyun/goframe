...
func TestRouteStaticFile(t *testing.T) {
	...
	_, err = f.WriteString("Gin Web goframe")
	...
	assert.Equal(t, w, w2)
	assert.Equal(t, http.StatusOK, w.Code)
	assert.Equal(t, "Gin Web goframe", w.Body.String())
...
func TestRouterMiddlewareAndStatic(t *testing.T) {
	...
	static := router.Group("/", func(c *Context) {
		...
		c.Writer.Header().Add("X-GIN", "Gin goframe")
	})
	static.Static("/", "./")
	...
	assert.Equal(t, "Gin goframe", w.Header().Get("x-GIN"))
}
