...
import (
	...
	"github.com/imajinyun/goframe/middleware/gzip"
)
...
func TestWrapHandler(t *testing.T) {
	...
	router.GET("/*any", WrapHandler(swgfiles.Handler, URL("https://github.com/swaggo/gin-swagger")))
...
func TestWrapCustomHandler(t *testing.T) {
	...
	router.Any("/*any", CustomWrapHandler(&Config{}, swgfiles.Handler))
...
func TestDisablingWrapHandler(t *testing.T) {
	...
	router.GET("/simple/*any", DisablingWrapHandler(swgfiles.Handler, disablingKey))
	...
	router.GET("/disabling/*any", DisablingWrapHandler(swgfiles.Handler, disablingKey))
...
func TestDisablingCustomWrapHandler(t *testing.T) {
	...
	router.GET("/simple/*any", DisablingCustomWrapHandler(&Config{}, swgfiles.Handler, disablingKey))

	assert.Equal(t, http.StatusOK, performRequest(http.MethodGet, "/simple/index.html", router).Code)

	_ = os.Setenv(disablingKey, "true")

	router.GET("/disabling/*any", DisablingCustomWrapHandler(&Config{}, swgfiles.Handler, disablingKey))
...
func TestWithGzipMiddleware(t *testing.T) {
	...
	router.GET("/*any", WrapHandler(swgfiles.Handler))
